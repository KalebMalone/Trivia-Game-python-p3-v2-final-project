...
def select_category(player):
    ...
    if choice not in categories:
        ...
        player.current_question_index = 0  # Reset question index
...
def check_answer(selected_question, user_answer, player):
    ...
    # Match the user's answer number to the correct answer text
    answer_mapping = {
...
def end_game(player):
    ...
    if choice == "1":
        player.questions_answered = 0  # Reset questions answered
        player.current_question_index = 0  # Reset question index
        select_category(player)  # Restart the game
    elif choice == "2":
